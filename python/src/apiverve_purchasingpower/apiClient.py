...
class PurchasingpowerAPIClientError(Exception):
...
class ValidationError(PurchasingpowerAPIClientError):
...
class PurchasingpowerAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"amount": {"type": "number", "required": True, "min": 0}, "from": {"type": "string", "required": True, "minLength": 4, "maxLength": 7}, "to": {"type": "string", "required": False, "minLength": 4, "maxLength": 7}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the PurchasingpowerAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises PurchasingpowerAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/purchasingpower'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise PurchasingpowerAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[PurchasingpowerAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns PurchasingPowerData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises PurchasingpowerAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise PurchasingpowerAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise PurchasingpowerAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise PurchasingpowerAPIClientError(f"Request failed: {str(e)}")
